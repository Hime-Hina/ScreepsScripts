...
The current smoke suite runs through a runner-owned suite/case/fixture registry. It contains `basic-runtime-heartbeat` and `memory-schema-write`, both using the `single-owned-spawn` fixture. P3 defense drill cases are selected explicitly through `node scripts/screeps-server/run-suite.mjs case <case>` and use separate hostile fixtures. Future growth should add runner cases and stable suite entrypoints, not a package script per strategy behavior.
...
- Defense drill cases: `defense-core-threat-safe-mode`, `defense-harmless-scout-continues`, `defense-distant-threat-defers-build`
...
- The `defense-harmless-scout` fixture contract is: active player `AliceBot`, room `W1N9`, spawn `Spawn1`, controller safe mode available, MOVE-only hostile creep `MichaelBot-harmless-scout` near core structures, a watched extension construction site, and adjacent owned workers with energy.
- The `defense-distant-threat` fixture contract is: active player `AliceBot`, room `W1N9`, spawn `Spawn1`, controller safe mode available, hostile creep `MichaelBot-distant-threat` away from core structures, hostile body containing active attack or dismantle capability, a watched extension construction site, and adjacent owned workers with energy.
- Defense status evidence must come from natural engine events in the generated status mod: safe mode intent, accepted intent, active controller state, post-heartbeat no-safe-mode observation, construction progress, or construction-deferred plus controller upgrade progress. The status mod observes engine hooks; it must not trigger safe mode or change game semantics itself.
...
| Core threat defense case lacks `safe-mode-active` evidence | Fail the case on watchdog; do not infer activation from source-level tests |
| Harmless scout defense case lacks post-heartbeat `defense-no-safe-mode` or construction progress evidence | Fail the case on watchdog |
| Distant threat defense case lacks post-heartbeat `defense-no-safe-mode` or construction-deferred evidence | Fail the case on watchdog |
...
- Good: `defense-harmless-scout-continues` observes a natural tick heartbeat, post-heartbeat `defense-no-safe-mode`, and watched construction site progress.
- Good: `defense-distant-threat-defers-build` observes a natural tick heartbeat, post-heartbeat `defense-no-safe-mode`, watched construction site progress remaining at its initial value, and controller upgrade progress.
...
- Defense case registry tests must assert each selected defense case maps to its explicit fixture and that mixed `single-owned-spawn` plus defense case lists are rejected before launch.
