...
- Harvesting worker priority is pickup dropped energy, then withdraw available energy, then harvest assigned source. Pickup, withdrawal, refill, repair, and harvest-source selection must use strategic metadata such as target type, remaining energy/work, hits ratio, position, and id only as a final tie-breaker.
...
- Multiple workers may build the same construction site in one tick while estimated reserved work does not cover remaining site work; do not reserve construction sites by id alone.
- Worker construction-site selection must be strategic and deterministic, not arbitrary id order: bootstrap infrastructure comes before roads, source-side road frontier comes before remote/controller-side roads, assigned-source-local source infrastructure comes before equivalent work near another source, and progressed equivalent sites continue before starting equivalent new ones.
- Runtime spawn snapshots must use room-level `Room.energyAvailable` and `Room.energyCapacityAvailable`, because `spawnCreep` consumes total spawn+extension energy.
- Runtime withdrawal snapshots must include neutral containers from `FIND_STRUCTURES`, not only owned structures, tombstones, and ruins.
- Early logistics road planning should create a small source-first frontier from source/container anchor toward spawn; avoid large route fan-out across source and controller paths in the same planning pass, and skip road path planning entirely when active construction-site backlog already blocks new low-priority roads.
