...
- 当前源码运行时行为是 tick heartbeat、Memory 边界和 RCL2 自持经济循环：runtime boundary 执行 `spawnCreep`、extension construction site 创建、worker 采集 source、补能 spawn/extension、critical spawn/extension/container/road repair、build construction site 和 controller upgrade action；worker 使用 `harvesting` / `working` 能量模式，避免 partial-energy builder 建一 tick 就回矿。
- 当前源码在 RCL2 extension 容量完成后可基于 room-level spawn+extension energy 孵化 550-cost `[WORK, WORK, CARRY, CARRY, CARRY, MOVE, MOVE, MOVE, MOVE]` worker；300 energy 时选择 `[WORK, CARRY, CARRY, MOVE, MOVE]`，只有 200 energy 可用时保留 `[WORK, CARRY, MOVE]` emergency worker。
- 当前源码会按 worker 名称与 source 位置/id 在同房间多 source 间确定性分配采集目标，避免所有 worker 固定选择第一个 source。
- 当前源码会在 RCL2 owned room 先为缺失 extension 规划 construction site；当 extension buildout 满足后，再基于 runtime snapshot 规划 source-adjacent container、controller-adjacent container，并用 source-first、anchor-to-spawn 的小 frontier 规划 early roads；active construction-site backlog 已超阈值时跳过 road path planning，只保留 container decisions；worker build target 使用结构类型、assigned-source locality、source 距离、progress 和位置排序，使分配到某个 source 的 worker 优先建设该 source 端的 container/road frontier；同 tick 多个 worker 可集中 build 同一 site 直到 estimated reserved work 覆盖 remaining work；继续维护已有 spawn/extension/container/road 的 critical repair fallback，且 worker 可从 neutral container withdrawal energy；暂不做 wall/rampart fortification、tower 或完整 base planner。
