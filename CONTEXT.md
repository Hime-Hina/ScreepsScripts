...
- 当前源码会在 RCL2 owned room 先为缺失 extension 规划 construction site；当 extension buildout 满足后，再基于 runtime snapshot 规划 source-adjacent container、controller-adjacent container，以及 spawn 到这些 logistics anchor 的最短 early roads，并继续维护已有 spawn/extension/container/road 的 critical repair fallback；暂不做 wall/rampart fortification、tower 或完整 base planner。
