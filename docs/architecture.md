...
`src/spawning/` 拥有 spawn 决策。当前 runtime spawn snapshot 使用 room-level `energyAvailable` / `energyCapacityAvailable`，所以 RCL2 extension 完工后可选择 550-cost early worker `[WORK, WORK, CARRY, CARRY, CARRY, MOVE, MOVE, MOVE, MOVE]`；300 energy 时选择 `[WORK, CARRY, CARRY, MOVE, MOVE]`，只有 200 energy 可用时保留 `[WORK, CARRY, MOVE]` emergency worker，并由 runtime boundary 执行 `spawnCreep`。

`src/construction/` 拥有纯 construction planner。当前 planner 会先为 RCL2 owned room 规划缺失的 5 个 extension construction site；当 extension buildout 满足后，再基于 runtime snapshot 中的 source/controller/terrain 规划 source-adjacent container、controller-adjacent container，并以 source-first、anchor-to-spawn 的小 frontier 方式规划 early logistics roads，避免一次性把 road sites 分散到多条路线；当 active construction-site backlog 已超过低优先级 road 阈值时，会先返回 container decisions 并跳过 road path planning。planner 会跳过 spawn、source、controller、已有结构、已有 construction site、edge 和 wall tile，并由 runtime boundary 执行 `Room.createConstructionSite`。

`src/creeps/` 拥有 bootstrap worker action 决策。当前 worker 按 creep name 和 source 位置/id 在同房间 source 之间做确定性分配；runtime boundary 捕获并持久化 `harvesting` / `working` 能量模式，partial-energy working creep 会继续工作到能量耗尽，partial-capacity harvesting creep 会继续采/取能到满；harvesting 分支按 dropped amount、withdraw target type/energy、距离和 id tie-breaker 选择 pickup/withdraw/harvest；working 分支优先补能 spawn/extension，refill 使用 structure type、gap、距离和 id tie-breaker；controller downgrade safe 时 build construction site 优先于 upgrade controller；controller downgrade recovering/warning 时同房间按 creep name 排序的第一个满能 worker upgrade 后其他满能 worker 可 build；critical 时所有满能 worker upgrade。P2 critical repair fallback 只维护已有 spawn、extension、container 和 road，优先级低于 spawn/extension refill 与 P0 controller downgrade guard，高于 ordinary build；repair target 使用结构类型、hits ratio、距离和 id tie-breaker。construction site build target 使用结构类型、assigned-source locality、source 距离、progress 和位置做确定性优先级：分配到某个 source 的 worker 会优先建设该 source 端的 source-local container/road；当 assigned source 没有 local site 时，再回退到确定性的 room-global 排序；同一 tick 允许多个 worker build 同一 site，直到 estimated reserved work 覆盖 remaining work。runtime boundary 捕获 owned controller `level` 和 `ticksToDowngrade`、owned room 结构 hits 快照、neutral container withdrawal energy，执行 harvest、transfer、withdraw、repair、build 和 upgradeController，并在 out of range 时执行 moveTo。
