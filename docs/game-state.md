...
- 2026-06-13 P3 local official server fallback：新增 `defense-core-threat-safe-mode` local runner case，fixture `defense-core-threat` 在本地官方 `screeps@4.3.0` server 中 seed `AliceBot / W1N9 / Spawn1`、controller safe mode 可用、near-core hostile creep `MichaelBot-core-threat`，hostile body 含 active `attack` 和 `work`；`node scripts/screeps-server/run-suite.mjs case defense-core-threat-safe-mode` 观察到自然 tick heartbeat 后记录 `safe-mode-active`，controller `safeMode=20002`、`safeModeAvailable=0`（`observed`，local official server e2e）。
...
当前重启房间、spawn、controller、source 采集、spawn/extension 补能、extension construction site 创建、worker build、P2 critical repair fallback 部署、P3 defense fallback 部署和远端代码 hash 已确认。P2 自然 repair action 证据暂时 blocked，原因是当前 supported repair backlog 为空。P3 live natural safe mode activation evidence 暂时 blocked，原因是当前 hostile creeps/spawns/towers 均为 `0`，safe mode charge `1` 未消耗。P3 PTR hostile drill blocked，原因是 PTR shard1 runtime CPU 尚未生效且不能在 CPU shard 冷却窗口内重提分配；local official server fallback 已验证 near-core hostile 会触发 safe mode。后续 road/container planner、wall/rampart fortification、tower 或更完整 base planning 应继续通过 Memory 边界和小行为切片进入。
